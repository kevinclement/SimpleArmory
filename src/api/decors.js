...
let _cache;
export async function getDecors(region, realm, character) {   
    if (!_cache) {
    ...
    console.log(`Parsing decors.json...`)
    ...
    // get json
    const db = await getJsonDb('decors');
    
    // get character collected
    const collected = await getData(region, realm, character, 'collections/decor');
    ...
    _cache.update(
        ...
        realm,
        character,
        parseCollectablesObject(db, profile, collected, 'decor_collected', 'decor')
    )
    return _cache.cache;
}